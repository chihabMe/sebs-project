...
export class BookingsService {
  ...
  private async notifyFollowersOnBooking(
    userId: string,
    ...
    if (!actor?.notifyFollowersOnBooking) return;
    ...
    await this.prisma.notification.createMany({
      data: follows.map((follow: any) => ({
        userId: follow.followerId,
        actorId: userId,
        type: "FOLLOWING_BOOKED_EVENT",
        title: `${actor.name} booked an event`,
        message: `${actor.name} booked "${event.title}"`,
        eventId: event.id,
  ...
  async create(
    ...
    const shouldAutoConfirm =
      Boolean(isViaInvite) || Boolean(event.autoApproveBookings);

    if (!isViaInvite && event.status !== "UPCOMING") {
      throw new BadRequestException(
        "Cannot book tickets for an event that is not upcoming",
      );
    }
      ...
      for (const question of event.formQuestions) {
        if (question.required) {
          ...
          if (!answer || !answer.answer) {
            throw new BadRequestException(
              `Answer for "${question.question}" is required`,
            );
          }
        }
      }
    ...
    if (existingBooking) {
      if (
        ...
        this.mailService
          .sendBookingRequestReceived(updatedBooking.user, updatedBooking.event)
          .catch(console.error);
        if (updatedBooking.status === "CONFIRMED") {
          this.mailService
            .sendBookingConfirmation(updatedBooking.user, updatedBooking.event)
            .catch(console.error);
        }
        await this.notifyFollowersOnBooking(userId, updatedBooking.event);
    ...
    this.mailService
      .sendBookingRequestReceived(booking.user, booking.event)
      .catch(console.error);
    if (booking.status === "CONFIRMED") {
      this.mailService
        .sendBookingConfirmation(booking.user, booking.event)
        .catch(console.error);
    }
    await this.notifyFollowersOnBooking(userId, booking.event);
  ...
  async checkIn(eventId: string, userId: string) {
    ...
    if (booking.attended) return booking;
  ...
  async getTicket(id: string, userId: string, userRole: string) {
    ...
    const pdfUrl = await this.ticketsService.generateTicketPDF(
      booking,
      booking.event,
      booking.user,
    );
  ...
  async cancel(id: string, userId: string) {
    ...
    this.mailService.sendBookingCancelled(booking.user, booking.event).catch(() => undefined);
    return updated;
  }
