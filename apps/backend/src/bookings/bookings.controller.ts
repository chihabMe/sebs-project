import { Controller, Get, Post, Body, Param, Patch, Query, UseGuards, ParseUUIDPipe } from '@nestjs/common';
...
export class BookingsController {
  ...
  async create(
    ...
    const booking = await this.bookingsService.create(userId, userRole, createBookingDto, query.token);
    return {
      success: true,
  ...
  async checkIn(@Param('eventId', new ParseUUIDPipe()) eventId: string, @GetUser('id') userId: string) {
  ...
  async cancel(@Param('id', new ParseUUIDPipe()) id: string, @GetUser('id') userId: string) {
  ...
  async checkStatus(@Param('eventId', new ParseUUIDPipe()) eventId: string, @GetUser('id') userId: string) {
